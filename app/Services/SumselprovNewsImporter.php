...
class SumselprovNewsImporter
{
    ...
    public function import(?int $maxPages = null): array
    {
        ...
        for ($page = 1; $page <= $maxPages; $page++) {
            $pageResult = $this->importPage($page);
    ...
    public function previewPage(int $page): array
    {
        $payload = $this->fetchPage($page);
    ...
    private function fetchPage(int $page): array
    {
        return Http::acceptJson()
            ->timeout(20)
            ->retry(3, 500)
            ->get(self::BASE_URL.'/api/sumselprov/api_berita_all2', ['page' => max(1, $page)])
            ->throw()
            ->json();
    }
